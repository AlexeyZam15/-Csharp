...
if (numberChoice1 == 0)
{
    int elementsMatrixRowSum = ElementsMatrixRowSum(matrix1, 2);
    Console.WriteLine($"Сумма элементов третьей строки массива = {elementsMatrixRowSum}");
}
else if (numberChoice1 == 1)
{
    Console.Write("Введите номер столбца: ");
    int columnIndex1 = Convert.ToInt32(Console.ReadLine());
    int elementsMatrixColumnSum = ElementsMatrixColumnSum(matrix1, columnIndex1-1);
    Console.WriteLine($"Сумма элементов {columnIndex1} строки массива = {elementsMatrixColumnSum}");
}
